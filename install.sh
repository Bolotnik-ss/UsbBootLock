...
# Скрипт установки и удаления UsbBootLock
# Этот скрипт автоматизирует процесс установки и удаления скрипта защиты загрузки.
# Он заменяет плейсхолдер серийного номера, настраивает систему и позволяет удалить установленные файлы.
...
# --- Функции для определения дистрибутива и путей ---
init_paths() {
...
uninstall_usb_lock() {
    echo "Начало удаления UsbBootLock..."

    # Проверяем, установлен ли скрипт, прежде чем удалять
    if [ ! -f "$INITRAMFS_TARGET_DIR/$HOOK_FILE_NAME" ]; then
        echo "Предупреждение: Скрипт UsbBootLock не найден в ожидаемой директории ($INITRAMFS_TARGET_DIR)."
        echo "Возможно, он уже удален или не был установлен корректно."
        # Тем не менее, попытаемся обновить initramfs на всякий случай.
    else
        echo "Удаление файла: $INITRAMFS_TARGET_DIR/$HOOK_FILE_NAME"
        sudo rm -f "$INITRAMFS_TARGET_DIR/$HOOK_FILE_NAME"
        if [ $? -ne 0 ]; then
            echo "Ошибка при удалении файла $INITRAMFS_TARGET_DIR/$HOOK_FILE_NAME."
            # Продолжаем, но информируем об ошибке
        fi
    fi

    # Если это Arch Linux, нужно удалить и хук, если он был создан отдельно
    if [[ "$HOOK_FILE_NAME" == "usb-lock.sh" && -d "$INITRAMFS_TARGET_DIR" && "$INITRAMFS_TARGET_DIR" == "/etc/initcpio/hooks/" ]]; then
        echo "Удаление хука mkinitcpio..."
        sudo rm -f "/etc/initcpio/install/usb-lock.sh" # Предполагаем, что файл хука назван так же
        if [ $? -ne 0 ]; then
            echo "Предупреждение: Не удалось удалить файл хука mkinitcpio."
        fi
    fi
    ...
    $INITRAMFS_UPDATE_CMD
    if [ $? -ne 0 ]; then
        echo "Ошибка: Не удалось обновить initramfs при удалении. Пожалуйста, проверьте вывод команды."
        exit 1
    fi

    echo ""
    echo "Удаление UsbBootLock завершено."
...
if [ "$1" == "--uninstall" ] || [ "$1" == "-u" ]; then
    init_paths # Инициализируем пути для определения дистрибутива
    uninstall_usb_lock
    exit 0
fi

# Если не "--uninstall", то выполняем установку
init_paths # Инициализируем пути для определения дистрибутива
install_usb_lock "$1"

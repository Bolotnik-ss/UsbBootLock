...
Для установки `UsbBootLock` вам потребуется клонировать репозиторий с GitHub и выполнить следующие шаги.
...
Чтобы удалить `UsbBootLock` из системы, используйте скрипт установки с опцией `--uninstall`:

1.  Перейдите в корневую директорию проекта `UsbBootLock`:
    ```bash
    cd UsbBootLock
    ```
2.  Запустите скрипт установки с флагом `--uninstall`:
    ```bash
    ./install.sh --uninstall
    ```

    Скрипт попытается определить вашу систему и автоматически удалить все компоненты, включая обновление `initramfs`. Если автоматическое удаление не удастся, следуйте инструкциям для ручного удаления:
